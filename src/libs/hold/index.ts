...
export const hold = <A>(stream: Stream<A>): Stream<A> => new Hold(stream);

export type HeldValue<A> = { value: A };

export class Hold<A> extends MulticastSource<A> {
  pendingSinks: Sink<A>[];
  held?: HeldValue<A>;
  task?: ScheduledTask;
